...
# ============================================
MODELS_DIR = "models"
MODEL_BASE_NAME = os.environ.get("TFT_MODEL_NAME", "modello_temporal_20260115_1726")
...
def load_model_and_scalers(model_base_name, models_dir):
    ...
    model = TemporalFusionTransformer(
        ...
        horizon_weights=config.get("horizon_weights", None)
    ).to(device)
...
def make_prediction(model, scalers, config, data_inputs, device):
    ...
    if num_quantiles > 1:
        ...
        predictions_scaled_back = preds_unscaled
    else:
...
def normalize_predictions_with_offset(predictions_raw, last_target_values, config):
    ...
    for t_idx, target_col in enumerate(target_columns):
        ...
        if predictions_raw.ndim == 3:
            predictions_normalized[:, t_idx, :] = predictions_raw[:, t_idx, :] + offset
        else:
            predictions_normalized[:, t_idx] = predictions_raw[:, t_idx] + offset
        ...
        for i in range(predictions_normalized.shape[0]):
            if predictions_raw.ndim == 3:
                raw_v = float(predictions_raw[i, t_idx, 0])
                nrm_v = float(predictions_normalized[i, t_idx, 0])
            else:
                raw_v = float(predictions_raw[i, t_idx])
    ...
    return predictions_normalized, normalization_info


def append_feature_importance_to_gsheet(gc, sheet_id_str, importance_sheet_name, attention_info, config):
    """Export feature importance (variable selection weights) to Google Sheets."""
    ...
    past_var_weights = attention_info["past_variable_weights"]
    ...
    avg_weights = weights_np.squeeze(0).mean(axis=0)
...
def append_predictions_to_gsheet(gc, sheet_id_str, predictions_sheet_name,
    ...
    for i in range(predictions_raw_np.shape[0]):
        ...
        if has_normalization:
            ...
            main_data = predictions_raw_np[i]

        if main_data.ndim > 1:
            main_data = main_data.flatten()
        row.extend([f"{val:.3f}".replace('.', ',') for val in main_data])

        if has_normalization:
            raw_data = predictions_raw_np[i]
            if raw_data.ndim > 1:
...
def main():
    ...
    try:
        ...
        # =====================================================
        # NORMALIZZAZIONE OFFSET SEDIMENTO
        # Ancora le previsioni al valore attuale dell'idrometro
        # preservando la tendenza prevista dal modello
        # =====================================================
        ...
        append_feature_importance_to_gsheet(
            gc, GSHEET_ID, GSHEET_FEATURE_IMPORTANCE_SHEET_NAME,
            attention_info, config
        )
