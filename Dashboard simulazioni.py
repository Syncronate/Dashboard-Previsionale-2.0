...
with st.sidebar:
    ...
    for i, opt in enumerate(radio_options):
        ...
        if opt == 'Dashboard': caption = "Monitoraggio GSheet"
            ...
            else: caption = f"Esegui previsioni ({active_config_sess.get('model_type', 'N/A')})"
        ...
        elif opt == 'Post-Training Modello LSTM': # NUOVA OPZIONE
            if not model_ready or active_config_sess.get('model_type') != 'LSTM': caption = "Richiede Modello LSTM attivo"; disabled = True
            elif not data_ready_csv: caption = "Richiede Nuovi Dati CSV"; disabled = True
            else: caption = "Affina un modello LSTM esistente"
...
# --- PAGINA SIMULAZIONE ---
    ...
    if active_model_type == "Seq2Seq":
         ...
         # Per la simulazione Seq2Seq, le feature storiche e forecast sono quelle *effettive* che il modello si aspetta.
         # Non applichiamo FE qui perché i dati GSheet dovrebbero già essere pronti o mappati direttamente.
         # Se il modello Seq2Seq è stato addestrato con FE, le colonne in `past_feature_cols_model` dovrebbero già includere i nomi ingegnerizzati.
         # Il mapping GSheet dovrebbe quindi mappare alle colonne *finali* attese dal modello.
         required_model_cols_for_mapping = past_feature_cols_model + [date_col_model_name] # Colonne finali che il modello si aspetta per l'encoder
         ...
         if past_data_loaded_s2s:
             ...
             for col in forecast_feature_cols_model:
                  ...
                  elif col in last_known_past_data.index and col in forecast_feature_cols_model : forecast_df_initial[col] = last_known_past_data.get(col, 0.0) # Usa valore passato se la feature è presente anche lì (es. livello)
         ...
         # Per la simulazione LSTM, le feature_columns_model sono quelle *effettive* che il modello si aspetta.
         # Se il modello è stato addestrato con FE, queste includono i nomi ingegnerizzati.
         # L'utente dovrà fornire dati per queste feature finali.
         # Se usa "CSV Caricato" o "GSheet", i dati devono contenere queste colonne finali o essere mappabili ad esse.
         # Se usa "Manuale" o "Tabella", inserirà valori per queste colonne finali.
         
         if sim_method == 'Manuale (Valori Costanti)':
             st.markdown(f'Inserisci valori costanti per le **{len(feature_columns_model)}** feature di input (effettive).'); st.caption(f"Questi valori saranno ripetuti per i **{input_steps_model}** passi temporali ({input_steps_model*0.5:.1f} ore) richiesti dal modello.")
             temp_sim_values_lstm = {}; cols_manual_lstm = st.columns(3); col_idx_manual = 0; input_valid_manual = True
             for feature in feature_columns_model: # feature_columns_model sono le model_input_feature_columns
             ...
             st.markdown(f'Importa gli ultimi **{input_steps_model}** steps ({input_steps_model*0.5:.1f} ore) da Google Sheet per le **{len(feature_columns_model)}** feature di input (effettive).'); st.caption(f"Verranno recuperati i dati dal Foglio Google (ID: `{GSHEET_ID}`).")
             date_col_model_name = st.session_state.date_col_name_csv
             ...
             # Aggiungi qui il mapping per eventuali feature ingegnerizzate se i loro nomi sono noti e standard
             # Esempio: 'Arcevia - Pioggia Ora (mm)_lag1h': 'Arcevia - Pioggia Ora (mm)_lag1h' (identico se il nome è già quello finale)
             # Questa parte è complessa se i nomi delle FE non sono standardizzati o se il GSheet non li contiene.
             # Per ora, si assume che feature_columns_model contenga i nomi che possono essere mappati direttamente.
             relevant_model_cols_lstm_map = feature_columns_model + [date_col_model_name]
             ...
             if missing_lstm_features_in_map: st.caption(f"Feature LSTM (effettive) non mappate da GSheet: `{', '.join(missing_lstm_features_in_map)}`. Saranno imputate.");
             ...
             sim_data_input_lstm_gs = None; sim_start_time_lstm_gs = None # Rinominato da start_pred_time_lstm
             if isinstance(imported_df_lstm_gs, pd.DataFrame):
                 try:
                     ...
                     sim_start_time_lstm_gs = st.session_state.get("imported_sim_start_time_gs_lstm", datetime.now(italy_tz)) # Usa timestamp salvato
             ...
             if st.button('Esegui Simulazione LSTM (GSheet)', type="primary", disabled=(not input_ready_lstm_gs), key="sim_run_exec_lstm_gsheet"):
                  if input_ready_lstm_gs:
                      with st.spinner('Simulazione LSTM (GSheet) in corso...'):
                            if isinstance(active_scalers, tuple) and len(active_scalers) == 2: predictions_sim_lstm = predict(active_model, sim_data_input_lstm_gs, active_scalers[0], active_scalers[1], active_config, active_device); start_pred_time_lstm = sim_start_time_lstm_gs # Usa timestamp corretto
                            else: st.error("Errore: Scaler LSTM non trovati o in formato non valido."); predictions_sim_lstm = None
                  else: st.error("Dati input da GSheet non pronti o non importati.")

         elif sim_method == 'Orario Dettagliato (Tabella)':
             st.markdown(f'Inserisci i dati per i **{input_steps_model}** passi temporali ({input_steps_model*0.5:.1f} ore) precedenti.'); st.caption(f"La tabella contiene le **{len(feature_columns_model)}** feature di input (effettive) richieste dal modello.")
             ...
             if data_ready_csv and len(df_current_csv) >= input_steps_model:
                 try: 
                     # Tenta di precompilare usando le feature_columns_model (effettive)
                     cols_to_extract_from_csv = [col for col in feature_columns_model if col in df_current_csv.columns]
                     if len(cols_to_extract_from_csv) == len(feature_columns_model): # Tutte le feature effettive sono nel CSV
                        editor_df_initial = df_current_csv[feature_columns_model].iloc[-input_steps_model:].reset_index(drop=True).astype(float).round(2)
                        st.caption("Tabella precompilata con gli ultimi dati CSV (usando feature effettive).")
                     else: # Non tutte le feature effettive sono nel CSV (es. FE complesse non presenti)
                        st.caption("Impossibile precompilare con tutte le feature effettive. Inizializzata a 0."); editor_df_initial = editor_df_initial.fillna(0.0)
             ...
             st.markdown(f"Utilizza gli ultimi **{input_steps_model}** steps ({input_steps_model*0.5:.1f} ore) dai dati CSV caricati, usando le feature effettive richieste dal modello.")
             ...
             try:
                 ...
                 # Verifica che *tutte* le feature_columns_model (effettive) siano nel CSV
                 missing_cols_csv = [col for col in feature_columns_model if col not in latest_csv_data_df.columns]
                 if missing_cols_csv:
                     st.error(f"Le seguenti feature effettive richieste dal modello non sono presenti nel file CSV: `{', '.join(missing_cols_csv)}`. Non è possibile usare questa opzione a meno che il CSV non contenga già le feature ingegnerizzate con questi nomi, o se il modello è stato addestrato senza FE.")
                 else:
                     sim_data_input_lstm_csv_ordered = latest_csv_data_df[feature_columns_model]
                     ...
                     st.caption("Dati CSV pronti per la simulazione."); st.expander("Mostra dati CSV utilizzati (Input LSTM)").dataframe(latest_csv_data_df[[date_col_name_csv] + feature_columns_model].round(3)) # Mostra anche data
         ...
         # Visualizzazione risultati LSTM (comune a tutti i metodi LSTM)
         ...
         elif any(st.session_state.get(f"sim_run_exec_lstm_{method.split()[0].lower()}", False) for method in sim_method_options if "LSTM" in method) : # Check if any LSTM sim button was pressed
...
# --- PAGINA TEST MODELLO SU STORICO ---
    ...
    # ... (codice Test Modello invariato) ...
    st.header('Test Modello su Dati Storici CSV (Walk-Forward Evaluation)')
    if not model_ready: st.warning("Seleziona un Modello attivo dalla sidebar per eseguire questo test."); st.stop()
    if not data_ready_csv: st.warning("Dati Storici CSV non disponibili. Caricane uno dalla sidebar."); st.stop()
    ...
    else: st.caption(f"Dati CSV Caricati: {len(df_current_csv)} righe. Colonna data non valida o assente per range.")

    target_columns_model_test = active_config['target_columns']
    if active_model_type == "Seq2Seq":
        encoder_features_key_test = "encoder_input_feature_columns" if "encoder_input_feature_columns" in active_config else "all_past_feature_columns"
        decoder_features_key_test = "decoder_input_feature_columns" if "decoder_input_feature_columns" in active_config else "forecast_input_columns"
        past_feature_cols_model_test = active_config[encoder_features_key_test]
        forecast_feature_cols_model_test = active_config[decoder_features_key_test]
        ...
        features_key_lstm_test = "model_input_feature_columns" if "model_input
