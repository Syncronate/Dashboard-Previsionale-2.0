...
# ============================================
MODELS_DIR = "models"
MODEL_BASE_NAME = os.environ.get("TFT_MODEL_NAME", "modello_temporal_20260519_1313")
...
GSHEET_PREDICTIONS_SHEET_NAME = os.environ.get("GSHEET_PREDICTIONS_SHEET", "Previsioni Thunder-ECMWF 24h")
GSHEET_FEATURE_IMPORTANCE_SHEET_NAME = os.environ.get("GSHEET_IMPORTANCE_SHEET", "TFT Feature Importance ECMWF 24h")
...
def fetch_and_prepare_data(gc, sheet_id, config):
    ...
    def build_rename_map(columns):
        ...
        for col in columns:
            new_name = col
            if 'Giornaliera ' in new_name:
                new_name = new_name.replace('Giornaliera ', '')
            if '_cumulata_30min' in new_name:
                new_name = new_name.replace('_cumulata_30min', '')
            
            if new_name != col:
                if '_cumulata_30min' in col or ('Giornaliera ' in col and not col.strip().endswith(')')):
    ...
    # Manual renaming for mismatching sensor names
    df_historical_raw.rename(columns={
        "Livello Idrometrico Sensore 1283 [m] (Corinaldo/Nevola)": "Livello Idrometrico Sensore 1283 [m] (Corinaldo)"
