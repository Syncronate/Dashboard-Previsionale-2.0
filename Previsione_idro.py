...
def append_predictions_to_gsheet(gc, sheet_id_str, predictions_sheet_name, predictions_np, target_columns, prediction_start_time, config):
    ...
    if rows_to_append:
        ...
        try:
            ...
            for s_info in sheet_info.get('sheets', []):
                if s_info.get('properties', {}).get('sheetId') == sheet_id_numeric:
                    for chart in s_info.get('charts', []):
                        delete_chart_requests.append({"deleteEmbeddedObject": {"objectId": chart['chartId']}})
                    break
            ...
            for i, target_name in enumerate(header_parts_targets):
                ...
                chart_series_requests.append({
                    "series": {
                        "sourceRange": { "sources": [{"sheetId": sheet_id_numeric, "startRowIndex": start_row_index_api, "endRowIndex": end_row_index_api, "startColumnIndex": series_column_idx_api, "endColumnIndex": series_column_idx_api + 1}] }
                    },
            ...
            chart_request = {
                "addChart": {
                    "chart": {
                        ...
                        "position": {"overlayPosition": {"anchorCell": {"sheetId": sheet_id_numeric, "rowIndex": 0, "columnIndex": len(header_row) + 1}, "offsetXPixels": 10, "offsetYPixels": 10, "widthPixels": 650, "heightPixels": 400}}
                    }
                }
            }
            service.spreadsheets().batchUpdate(spreadsheetId=spreadsheet_id, body={"requests": [chart_request]}).execute()
            print("Grafico aggiunto/aggiornato con successo.")
        ...
        print(f"Nessuna riga di previsione da aggiungere.")
